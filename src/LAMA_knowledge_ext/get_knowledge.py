...
parser.add_argument("--model_path", type=str, default='models/finetuned_gpt_0.0.pt', help="Path to the fine-tuned GPT model")
parser.add_argument("--output_knowledge", type=str, default='LAMA_knowledge_ext/results/knowledge_0.0.json', help="Path to save the extracted knowledge")
parser.add_argument("--output_similarity", type=str, default='LAMA_knowledge_ext/results/similarity_0.0.json', help="Path to save the similarity results")
...
with open("LAMA_knowledge_ext/data/ConceptNet/test.jsonl", "r") as file:
...
with open("LAMA_knowledge_ext/data/Google_RE/date_of_birth_test.jsonl", "r") as file:
    statements = [json.loads(line) for line in file]

querie_new = [(f'{q["sub_label"]} was born in', q["obj_label"]) for q in statements]
queries += random.sample(querie_new, min(nr_queries, len(querie_new)))

with open("LAMA_knowledge_ext/data/Google_RE/place_of_birth_test.jsonl", "r") as file:
    statements = [json.loads(line) for line in file]

querie_new = [(f'{q["sub_label"]} was born in', q["obj_label"]) for q in statements]
queries += random.sample(querie_new, min(nr_queries, len(querie_new)))

with open("LAMA_knowledge_ext/data/Google_RE/place_of_death_test.jsonl", "r") as file:
    statements = [json.loads(line) for line in file]
